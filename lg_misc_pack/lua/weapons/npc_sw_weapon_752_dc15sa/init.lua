...
function SWEP:Initialize()
	...
	if self.Owner:GetClass() == "npc_combine_s" then
		...
		local crouchseq = self.Weapon.Owner:LookupSequence("cover_crouch_low") 
			...
			if self.Weapon.Owner:LookupSequence("pistolangryidle2") == nil then return end
		local manhackseq = self.Weapon.Owner:LookupSequence("pistolangryidle2") 
				if self.Weapon.Owner:GetSequenceName(manhackseq) == "pistolangryidle2" then
